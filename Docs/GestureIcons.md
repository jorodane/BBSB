...
- 안내 아이콘은 **화면 하단 중앙**에 모아 표시한다. 현재 Call이 시작된 몬스터만 자리를 차지하고 최고난도에서 안내가 겹치면 옆으로 나란히 배치한다. 판정 결과도 같은 중앙 영역 아래에 표시한다. 전투 HUD는 기존 결과 집계를 이 위치에 표시하고, 연습·도감은 무대의 판정 표시를 사용한다.
