...
플레이어·몬스터·무기는 기존 네이티브 프리팹과 아트를 사용하며, 배경과 HUD/일반 노트 그래픽 9종에 공통 적 공격 노트 1종을 추가한다.
...
기존 테마 에셋과 수동 편집값은 재설치로 덮어쓰지 않는다. 추가 팩 `BBSB_Attack_Note_Update.zip`의 `Assets` 폴더를 합치면 기존 테마에 비어 있는 `enemyAttackNote`만 한 번 연결한다. 이후 일부러 참조를 비우면 자동으로 다시 채우지 않는다. PNG를 같은 경로에서 교체하면 기존 참조가 유지된다.
...
| `enemy-attack-note.png` | 전열·후열 공통 붉은 공격 노트. Hold의 머리와 끝에도 사용 |
...
- 노트는 아래에서 위로 일정하게 이동하며 전투 필드 바로 아래의 판정선에 도착한다. 박자에 따른 가속·감속은 없다. 3박 미리보기와 전투 판정·피해·무기 패턴·카운트인 시간은 유지한다.
- 레인 상단에는 키만 남긴다. 무기 이름과 상태 설명을 제거하고 실제 부유 무기는 유지한다.
- 적 공격 노트는 진형과 관계없이 같은 이미지를 사용한다. 공격 Hold는 연결된 붉은 몸체로 지속 시간을 표시한다. 공격 종류에 따른 구분은 추후 추가한다.
