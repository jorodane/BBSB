...
**12종·24패턴·36개 원본 공격 단계**에 업로드된 몬스터걸 본체 93개·공격 PNG 223개와 음악 시계 기반 동작이 연결되어 있다. 준비 화면의 연습·전투·도감이 같은 본체 포즈와 공격 이미지 경로를 사용한다. 파일이 없는 공격은 종족별 도형으로 표시한다.

이미지를 제작할 때는 [Sol용 파일별 제작 명세](MonsterArtProduction.md)를 사용한다. 최종 캔버스 크기·포즈·접점·재사용 원본을 정리했으며, [전체 PNG 배치 목록](MonsterArtFiles.json)에 각 파일의 정확한 경로와 복사 원본이 있다.
...
예: `clock-spirit/clock-seven-beat-wait/step-0/travel-0.png`~`travel-3.png`의 작은 인형이 7박 동안 걷는다. `tap-slime/count-four-tap/step-0/spawn.png`는 각 Call의 젤리 폭발에 사용된다. 모든 대상 폴더에 실제 PNG가 들어 있다.
...
`NamedResourceClips`는 번호 PNG → 정확한 단일 PNG 경로 → 기존 명명된 아틀라스 순서로 필요한 이미지부터 읽는다. 파일/클립 조회 결과를 캐시한다. Single PNG는 파일 경로로 식별하므로 Sprite 이름을 편집해도 연결이 유지된다. 자동 슬라이스의 `_0`, `_1` 이름을 애니메이션 프레임으로 간주하지 않는다.
