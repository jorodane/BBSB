...
- 첫 Call을 **1박**으로 센다. `clock-seven-beat-wait`는 8박에 Tap, `offbeat-single-tap`은 1.5박에 Tap한다. 입력 구간은 실시간 문구, 타임라인과 박자 목록으로 안내한다. 왼쪽 전투 미리보기 영역을 누르거나 드래그하면 플레이어가 반응하고 PERFECT·반미스·MISS를 바로 표시한다.
