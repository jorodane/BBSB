...
시각은 **0부터 시작하는 상대 박자**이며 0.25박 단위로 반올림한다. Call 시각은 Call 시작 기준이고 Response 시각은 Response 시작 기준이다. 첫 Call의 상대 시각은 0이어야 한다. 첫 Response에는 빈 박자를 둘 수 있다. Call은 실제 첫 입력 전에 모두 끝나야 한다. 편집 구간이 먼저 시작하더라도 마지막 Call 뒤 입력이면 유효하다.
