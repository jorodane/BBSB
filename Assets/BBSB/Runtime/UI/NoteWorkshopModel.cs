...
namespace BBSB.Runtime.UI
{
    ...
    public enum NoteDemoInput { Idle, Press, Hold, Release }
    ...
    public sealed class NoteWorkshopPlayback
    {
        ...
        public NoteWorkshopPlayback(WeaponPhrase phrase, int width, int offset)
        {
            ...
            LoopBeats = phrase.LengthBeats * CyclesPerLoop +
                (phrase.Repeat && phrase.MaximumCycles == 0 ? 0 : Math.Max(1, phrase.CompletionCooldownBeats));
            foreach (var note in phrase.Notes)
            {
                ...
                else inputs.Add(new Input { Start = note.Beat, End = note.Beat + note.HoldBeats, Lane = Lane(note), RequiresRelease = release });
            }
        ...
        public bool ConnectsFromPrevious(WeaponPhraseNote note, double start) => connected.Contains(note) ||
            (ReferenceEquals(note, Phrase.Notes[0]) && connectsCycles && start > 0 &&
             (start % LoopBeats > .000001 || LoopBeats == Phrase.LengthBeats * CyclesPerLoop));

        public double PatternBeat(double beat)
        {
            if (beat < 0) return -1;
            double local = beat % LoopBeats;
        ...
        public void VisitNotes(double from, double to, Action<WeaponPhraseNote, double, double, int> visit)
        {
            ...
            long first = Math.Max(0, (long)Math.Floor((from - Phrase.LengthBeats) / LoopBeats));
            long last = Math.Max(0, (long)Math.Floor(to / LoopBeats));
            for (long block = first; block <= last; block++)
                for (int cycle = 0; cycle < CyclesPerLoop; cycle++)
                    foreach (var note in Phrase.Notes)
                    {
                        double start = block * LoopBeats + cycle * Phrase.LengthBeats + note.Beat;
