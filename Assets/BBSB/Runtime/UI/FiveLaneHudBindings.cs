...
namespace BBSB.Runtime.UI
{
    ...
    public sealed class FiveLaneHudBindings : MonoBehaviour
    {
        ...
        // Old prefabs can retain these references so their description widgets are retired.
        [HideInInspector] public TextMeshProUGUI[] laneStatus = new TextMeshProUGUI[5];
        ...
        public bool TryValidate(out string problem)
        {
            ...
            else if (!Complete(weaponRoots) || !Complete(inputAreas) || !Complete(laneLabels) || !Complete(laneResults))
        ...
        public bool EnsureStageLayout()
        {
            ...
            for (int i = 0; i < 5; i++)
                ...
                {
                    var point = FiveLaneTrackGraphic.LanePoint(i, 1);
        ...
        public void ConfigureLanes(BBSB.Core.FiveLaneBattle battle)
        {
            ...
            for (int i = 0; i < BattleInputLayout.LaneCount; i++)
            {
                ...
                if (laneStatus[i] != null) laneStatus[i].gameObject.SetActive(false);
                laneResults[i].gameObject.SetActive(active);
                ...
                Place(laneResults[i].rectTransform, headerX - half, judgmentY - .043f, headerX + half, judgmentY - .008f);
                laneResults[i].fontSize = 15; laneResults[i].enableAutoSizing = true;
                laneResults[i].fontSizeMin = 9; laneResults[i].fontSizeMax = 15;
        ...
        private void EnsureExtensionSlots(InputExtensions extensions)
        {
            ...
            {
                float x = FiveLaneTrackGraphic.InputPoint(i, 1, extensions).x;
                ...
                if (judgmentPoints[i] == null)
                { judgmentPoints[i] = ui.Rect("Judgment point " + key, tracks); Place(judgmentPoints[i], x - .035f, .41f, x + .035f, .43f); }
