...
namespace BBSB.Core
{
    public sealed class StartingWeaponDefinition
    {
        ...
        public StartingWeaponDefinition(string key, string weaponId, int? defaultOffset, WeaponAttribute attribute = WeaponAttribute.Light)
        {
            if (string.IsNullOrWhiteSpace(key)) throw new ArgumentException("A starter needs a stable key.", nameof(key));
            WeaponCatalog.Find(weaponId); WeaponAttributes.Validate(attribute); Attribute = attribute;
    ...
    public sealed class RunCharacterDefinition
    {
        ...
        public RunCharacterDefinition(string id, IEnumerable<StartingWeaponDefinition> startingWeapons)
        {
            ...
            foreach (var entry in entries)
            {
                ...
                var weapon = new WeaponState(entry.WeaponId, entry.Attribute); defaults.Acquire(weapon);
