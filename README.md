...
Unity 모바일 버전의 탐험 흐름과 음악 슬롯 기반 구현. Unity **6000.3.5f2**, URP 2D, Input System을 사용한다.
...
5. 전투 노드에서는 선택된 샘플 곡을 확인한다. **개발용 슬롯 미리보기 → 슬롯 펼치기**에서 다섯 곡과 각 마디를 둘러볼 수 있다. 이 미리보기는 `Show Battle Test Controls`가 켜져 있을 때만 표시된다.
...
- 몬스터 패턴의 전체 리듬, 유지 길이, 전조 선행 시간을 만족하는 후보를 `IEnumerable<PatternPlacement>`로 조회할 수 있다. 실제 몬스터 배치/충돌 중재는 다음 단계다.
...
| 파일/폴더 | 역할 |
|---|---|
| `Assets/BBSB/Core` | Unity에 의존하지 않는 맵·음악 슬롯 생성, 패턴 후보 조회, 런 상태, 보상 규칙 |
| `Assets/BBSB/Runtime/RunBootstrap.cs` | 씬 진입점, 초기 값, 한글 폰트, 입력 모듈 |
| `Assets/BBSB/Runtime/RunPresenter.cs` | 화면 구성과 사용자 명령 전달 |
| `Assets/BBSB/Runtime/UI` | 레이아웃, 노드 연결선, 안전 영역, 개발용 음악 슬롯 미리보기 |
...
`BattleRequested`가 호출될 때 `Session.BattleMusic`에 선택된 곡과 슬롯이 준비되어 있다. 같은 전투의 준비/연주 반복은 이 객체를 재사용한다. 스테이지 종료, 게임오버, 런 재시작 시 해제된다. 개발용 샘플 곡 둘러보기는 실제 전투의 선택을 변경하지 않는다.
...
핵심 로직은 .NET 8에서 실행해 **28개 테스트 통과**. 기존 탐험 테스트 17개에 음악 관련 11개를 추가했다. 다섯 곡의 재현성, 정박/엇박 간격 보존, 유지 길이, 매 패턴의 전조, 하이라이트 가중치, 곡 끝/빈 구간 경계, Dive-Shake-Flick 조합의 슬롯 조회, 곡 선택과 수명 관리를 검증한다. 기존 1,000개 시드의 맵 연결성, 1,003개 시드 × 4필드의 경로별 상점·휴식 제한과 보상/게임오버 규칙도 통과했다.
