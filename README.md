...
핵심 로직은 .NET 8에서 실행해 **17개 테스트 통과**. 1,000개 시드의 연결성/보스 위치, 1,003개 시드 × 4필드의 모든 경로별 상점·휴식 제한, 세로 연결 제거와 분기 유지, 필드 전환 시 제한 초기화를 검증한다. 진입 제한, 보상 중복 방지, 늦게 도착한 전투 결과, 무기 교체, 상점, 회복, 강화, 필드 진행, 게임오버 초기화도 포함한다.
